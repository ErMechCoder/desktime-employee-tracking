...
const Navbar = (props) => {
  ...
  // let id = x.user._id;
  let useremail = x.user.email;
  const s = useremail;
 const username = s.slice(0, s.lastIndexOf("@"));
  ...
  return (
    <Div1>
      <Div2>
        ...
        <Link to='/'><img src={"https://ekanatechnologies.com/wp-content/uploads/2023/03/ekana_logo-361.png"} alt="ekana logo" width="150px" height="27px" /></Link>
      </Div2>
      <Div3>
        <Menu isLazy>

          <MenuButton><Box display="flex"> <Text>{username} Workspace</Text><img src={dropdown} alt="error" width="30px" /></Box></MenuButton>
          <MenuList>
        ...
        <Menu isLazy w="10%">
          <MenuButton p="8px" borderRadius="50%" backgroundColor="#03a9f4" color="white"  >{username}</MenuButton>
          <MenuList>
            <MenuItem>{ useremail}</MenuItem>
            ...
            <MenuItem><Link to={"/login"}>Login Out</Link></MenuItem>
          </MenuList>
        </Menu>
      </Div3>
    </Div1>
